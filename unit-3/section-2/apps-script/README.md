# Section 2 — Inside the Cell: Apps Script deployment
...
|---|---|---|
| `Code.gs` | Section 2 entry point. Holds the Sheet ID and the `doGet` handler. | Yes |
...
2. Rename the project to: `G9 Bio U3 S2 Web App`.
...
1. Open `SeedSheet.gs`, choose **`seedSection2Sheet`** from the function dropdown, click **Run**.
2. Authorize when prompted. The script needs permission to write to your Sheet.
3. Open the Sheet and check the tabs appeared: `meta`, `mcq`, `fill_blank`, `drag_columns`, `cell_compare`, `organelles`, `ai_critique_text`, `ai_critique_errors`. Each has a header row and data rows.
...
2. Description: `G9 Bio U3 S2 — v1`. Execute as **Me**. Who has access: **Anyone with the link**.
...
exists, add it to the steps listed for Inside the Cell in `g9-bio-site/INTAKE-NOTES.md`, Unit 3 note 3,
...
Run `exportSection2Snapshot` from the function dropdown. It writes
`content-snapshot-u3-s2.json` to your Drive root. Download it, replace
